import { ComponentType, StringSelectMenuBuilder, ActionRowBuilder, ButtonBuilder, EmbedBuilder, ChatInputCommandInteraction } from 'discord.js';
...
export const createNoPermsMessage = (interaction: ChatInputCommandInteraction, permsName: string) => {
	const embed = new EmbedBuilder()
		.setColor(0xff0000)
		.setTitle('권한 없음')
		.setDescription(`:x: </${interaction.commandName}:${interaction.commandId}>를 실행할 권한이 없어요!\n이는 ${permsName} 권한을 필요로 합니다.`)
		.setTimestamp();
	return embed;
};
...
export const permsChecker = async (
	...
	if (guildOnly && !interaction.inGuild()) {
		...
		const embed = new EmbedBuilder()
			.setColor(0xff0000)
			.setTitle('서버 전용')
			.setDescription(`:x: </${interaction.commandName}:${interaction.commandId}> 커맨드는 DM에선 실행할 수 없어요!\n언더바게임 서버에서 다시 실행해 주세요.`)
			.setTimestamp();
